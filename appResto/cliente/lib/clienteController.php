...
require_once '../lib/clienteModel.php';
...
class ClienteController {
    ...
    public function agregar_producto() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Crear una instancia del modelo ClienteModel
            $producto_model = new ClienteModel();
...
if(isset($_POST['enviar_pedido'])) {
    ...
    $pedidos = new clienteModel();

    // Verificar si al menos un producto ha sido seleccionado y se ha ingresado el número de mesa
    if(!empty($productos_id) && !empty($mesa)) {

        // Recorrer el array de id de productos
        foreach($productos_id as $producto_id) {
            // Verificar si el checkbox del producto está seleccionado
            if(!empty($_POST["producto_$producto_id"])) {
                // Verificar si los campos obligatorios no están vacíos
                if(!empty($mesa) && !empty($precio)) {
                    // Llamar al método agrega_pedidos_mesa del modelo
                    $pedidos->agrega_pedidos_mesa($producto_id, $mesa, $precio, $observacion);
                    echo "El pedido para el producto con id $producto_id se ha registrado correctamente.";
                } else {
                    // Mostrar mensaje de error si faltan campos obligatorios
                    echo "Por favor complete los campos obligatorios para el producto con id $producto_id.";
                }
            }
        }

    } else {
        // Mostrar mensaje de error si no se ha seleccionado ningún producto o no se ha ingresado el número de mesa
        echo "Por favor seleccione al menos un producto y complete el número de mesa.";
    }
